...
public class FDinMotion extends Configured implements Tool {
	public static class Map extends Mapper<Text, Image, Text, Image> {
		...
		public void map(Text key, Image value, Context context)
			...
			if(!classifier.isNull()){
				...
				context.write(key, new Image(image));
			  } //while close
		        //grabber.stop();
			}
				catch(Exception e) {
	...
	public static class Reduce extends Reducer<Text, Image, Text, Image> {
		...
		public void reduce(Text key, Iterable<Image> values, Context context)
			...
			//while (it.hasNext()) {
				Image frame = it.next();
				try {
					FrameRecorder recorder = FrameRecorder.createDefault("output.avi", frame.getWidth(), frame.getHeight());
					recorder.start();
					recorder.record(frame.getImage());
					while (it.hasNext()) {
						frame = it.next();
						recorder.record(frame.getImage());
					}
	...
	public int run(String[] args) throws Exception {
		...
		job.setJarByClass(FDinMotion.class);
	...
	public static void main(String[] args) throws Exception {
		...
		int res = ToolRunner.run(new Configuration(), new FDinMotion(), args);
		t = cvGetTickCount() - t;
